    ...
    if(world_size==1){
        ...
        while (!finished && n_itrs < MAX_ITRS){
            ...
            for (int i = 0; i < rows_held_by_process; i++){
                ...
                for (int j = 1; j < column_held_by_process; j++){
                    if (grid[i][j] == 1 && (grid[i][(j+1)%column_held_by_process] == 0)){
                        grid[i][j] = 0;
                        grid[i][(j+1)%column_held_by_process] = 3;
                    }
                    else if (grid[i][j] == 3)
                        grid[i][j] = 1;
                }
   ...
   // printf("TORUS SHORT %d\nTORUS LONG %d\n",torus_short,torus_long);
    ...
    if(color == 1){             //forget abandoned processes till the end
        ...
        while (!finished && n_itrs < MAX_ITRS){
            ...
            /*
             2 routines for red movement
             a) when using a 1D array of processes  -- no communication for RED, communication for BLUE ONLY
             b) when using a 2D TORUS, use communication for RED communication
            */
            ...
            if(torus_short==1){
                ...
                for (int i = 0; i < number_of_rows; i++){
                    ...
                    for (int j = 1; j < number_of_columns; j++){
                        if (grid[i][j] == 1 && (grid[i][(j+1)%number_of_columns] == 0)){
                            grid[i][j] = 0;
                            grid[i][(j+1)%number_of_columns] = 3;
                        }
                        else if (grid[i][j] == 3)
                            grid[i][j] = 1;
                    }
